	...
	public function followers($id)
	{
		...
		*/
		...
		if (!empty($this->input->post(NULL, true)) && $this->input->post('change_follower_options')) {
			...
			// scoped to the same follower trigger so it reflects the Extra Balls section shown below
			$extra_numbers_str = '';
			if(!empty($this->data['lottery']->duplicate_extra_ball) && !empty($this->data['lottery']->extra_ball)) {
				...
				$generated_extra = $this->predictions_m->followers_extra_prediction($id, $extra_pool, $follower_type, $follower_select);
		...
		if ($followers_record && !empty($this->data['lottery_numbers'])) {
			...
			if ($_stale) {
				...
				if(!empty($this->data['lottery']->duplicate_extra_ball) && !empty($this->data['lottery']->extra_ball)) {
					...
					$_gen_extra = $this->predictions_m->followers_extra_prediction($id, $_extra_pool, $_curr_type, $_follower_select);
	...
	*/
	public function recalc_followers($id, $lotto)
	{
		...
		if(!$use_sliding_window && !is_null($followers))
		{
			...
			// NOTE: update_followers_cumulative_wins() is DISABLED to prevent interference with real-time win tracking
			// Real win statistics are updated automatically during import via check_and_update_followers_wins()
	...
	private function auto_regenerate_followers_prediction($id)
	{
		...
		// STEP 5: Generate extra ball predictions independently of the main result — the extra ball
		...
		if ($generated || $extra_numbers_str !== '') {
			// STEP 6: Save the regenerated prediction (main numbers preserved as-is if regeneration failed)
			...
			$this->statistics_m->followers_prediction_save(
				...
				$generated ? $generated : $followers_record['lottery_numbers'],
	...
	private function calculate_hwc_wins($lottery_id, $range, $prediction_pool, $hots, $warms, $colds, $extra_included = false, $extra_draws = false)
	{
		try {
			...
			if ($wins_string !== FALSE) {
				...
				// NOTE: update_hwc_cumulative_wins() is DISABLED to prevent interference with real-time win tracking
				// Real win statistics are updated automatically during import via check_and_update_hwc_wins()
