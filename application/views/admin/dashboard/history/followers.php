				...
				<i class="fa fa-trophy"></i> Followers Prediction Win Records
						...
						<div style="margin-bottom: 10px; text-align: center; font-size: 0.9em; color: #666;">
							<?php if($followers_win_stats['startdate'] || $followers_win_stats['lastdate']): ?>
								<strong><?php echo ($followers_win_stats['lastdate'] || $followers_win_stats['total_winners'] > 0) ? 'Start:' : 'Starting Date:'; ?></strong> <?php echo $followers_win_stats['startdate'] ? date('l F j, Y', strtotime($followers_win_stats['startdate'])) : date('l F j, Y', strtotime($lottery->next_draw_date)); ?>
								&nbsp;&nbsp;|
								<?php if($followers_win_stats['lastdate']): ?>
									<strong>Last Draw Date:</strong> <?php echo date('l F j, Y', strtotime($followers_win_stats['lastdate'])); ?> (<?php echo $draw_count; ?> draw<?php echo $draw_count != 1 ? 's' : ''; ?>)
								<?php else: ?>
	...
	if(confirm('WARNING: This will clear all Followers prediction win records and reset the statistics.\n\nThe new start date will be set to the next draw date.\n\nAre you sure you want to continue?')) {
		$.ajax({
			...
			success: function(response) {
				if(response.success) {
					alert(response.message || 'Followers win statistics reset successfully');
					location.reload();
				} else {
